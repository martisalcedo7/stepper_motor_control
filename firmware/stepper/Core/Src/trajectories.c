...
void smooth_trajectory(float xf, float yf, float current_theta_1, float current_theta_2, uint16_t array_size, float *x_positions, float *y_positions)
{
    ...
    {
        float t = idx * PERIOD;
        x_positions[idx] = _smooth_trajectory_point(xf, xi, t, total_time);
        y_positions[idx] = _smooth_trajectory_point(yf, yi, t, total_time);
    }
}
...
void smooth_trajectory_and_inverse_kinematics(float xf, float yf, float current_theta_1, float current_theta_2, uint16_t array_size, float *theta_1, float *theta_2)
{
    ...
    {
        float t = idx * PERIOD;
        float x_position = _smooth_trajectory_point(xf, xi, t, total_time);
        float y_position = _smooth_trajectory_point(yf, yi, t, total_time);
    ...
    while(*current_position_index < (positions_size - 1)){
        ...
        if ((pos_diff > 0 && next_pos < (positions[*current_position_index + 1] - positions[0])) || 
            (pos_diff < 0 && next_pos > (positions[*current_position_index + 1] - positions[0]))) {
                ...
                float section_time_stepper = PERIOD * (next_pos - (positions[*current_position_index] - positions[0])) / pos_diff;
                float time_stepper = PERIOD * (*current_position_index) + section_time_stepper;
                *time_increment = time_stepper - *accumulated_time_stepper;
                *accumulated_time_stepper += *time_increment;
